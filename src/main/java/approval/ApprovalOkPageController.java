...
public class ApprovalOkPageController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//전체 게시물 개수
		int totalCount = dao.countAllOk(map);
		...
		map.put("end", end);
		//DB에서 게시물 정보 읽기
		List<ApprovalDTO> boardList = dao.getListOkPage(map, id);
		dao.close();
		

		String pageingStr = PagingUtil.pagingCenter(totalCount, pageSize, pageBlock, pageNum, "../approval/approvalOkPage.do");
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//DAO를 통해 DB에 내용 저장
		ApprovalDAO dao = new ApprovalDAO();
		int result = dao.okPostUpdate(dto);
		...
		if(result==1 && Dresult==1) { //성공
			AlertFunc.alertLocation(resp, "승인 했습니다.승인 페이지로 이동합니다.", "../approval/approvalOkPage.do");
		}else {
			AlertFunc.alertLocation(resp, "승인 되지 않았습니다.", "../approval/view.do?idx="+idx);		
		}
